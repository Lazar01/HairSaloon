...
import { deleteEmployee } from "../../fetchData";
interface ModalProps {
  setShowModal: (showModal: boolean) => void;
  id: number;
  refetch: () => void;
}
const DeleteStaffModal: React.FC<ModalProps> = ({
  setShowModal,
  refetch,
  id,
}) => {
  const { data, error, loading, getData } = deleteEmployee();
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
              <h3 className="sm:text-base md:text-3xl font-semibold">
                Are you sure that you want to remove this member of staff?
              </h3>
              <button
